...
\begin{document}
	...
	\begin{abstract}
		...
		% ...approximation "of" in the objective loss function (bin mir da nicht ganz sicher, ob das inhaltlich noch richtig ist; müsste glaube ich aber auf jeden Fall approximation "of", statt "for" sein)
	...
	% Müsste heißen "...is not being considered" oder "One problem of the SGD method is that it does not consider the pathological curvature of the OL."
	...
	% On the other hand $2^{\text{nd}}$-order optimization methods take the curvature of the OL into account and can thus circumvent such problems. 
	But 
	%However, ...
	...
	%glaube "composition" ist üblicher zu verwenden
	...
	% For cases in which no further mapping of the network output is needed, we also allow $\psi$ to be the identity function.


	\subsection{Loss functions}
	\noindent
	Choosing a good 
	% welll-suited statt good
	Loss function
	\begin{align}
	...
	%For a fixed observation pair $(x, y)$ with current parameters $\theta$, the output loss takes $\hat{y}_{x} = \phi(R_{x}(\theta))$ as input.
	...
	%and neglect the dependency to the parameters $\theta$ in the notation...
...
% !!! Erst ab hier die neueste Version benutzt !!!
...
% verstehe den Satz nicht ganz. Normalerwiese müsste es zumindest consider...as... sein
...
%Hier fehlt ein "that". Bin mir bei der Aussage des Satzes nicht ganz sicher. Haben wir eine kleine batch size gefunden, mit der es gilt-->we found a small batch size that... ; herausgefunden, dass es es mit einer kleinen batch size passt--> we found out, that a small batch size.. 
...
\begin{thebibliography}{00}
	...
	\bibitem{b1} O. Chapelle, D. Erhan, "Improved Preconditioner for Hessian Free Optimization", Yahoo! Labs
